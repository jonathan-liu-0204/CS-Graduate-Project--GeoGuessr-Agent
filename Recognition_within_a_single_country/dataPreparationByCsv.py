...
def downloadImg(imgCount, targetState, usage, latRange, latMin, lngRange, lngMin):
    ...
    if checkDetail["status"] == "OK": # 回傳值為OK代表合法經緯度，若為REQUEST_DENIED or ZERO_RESULTS則為非法
        ...
        # if(countryDetail["countryName"] == targetState): # 若此合法經緯度位於我們設定的目標國家
        #     viewRresponse = requests.get(viewUrl)
        #     img = Image.open(BytesIO(viewRresponse.content)) # 轉成 img 供以後做處理
        #     img.save("./generated/images/" + usage + "/" + targetState + '/' + location + ".jpg") # 將圖片儲存在路徑 'generated/images/{country}/'
        #     print(f'Number: {currentCount+1}/{imgCount}\tLocation: {tuple((lat, lng))}\tState: {targetState}') # 印出其經緯度及對應的國家名稱 (供未來作為 ground truth)
