...
def downloadImg():
    ...
    print(f'Location: {location},\tStatus: {viewRresponse.status_code}')
    if viewRresponse.status_code == 200: # 回傳值為200代表合法經緯度，若為404則為非法
        ...
        countryDetail = json.loads(countryResponse.text) # 將回傳的 json 格式檔毒讀入
        print(countryDetail["countryName"]) # 印出當中的國家名稱 (供未來作為 ground truth)
        return True
    return False
